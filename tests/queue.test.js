...
test.describe('Queue', () => {
    ...
    test('should reject partial custom queue hooks', () => {
        ...
        assert.throws(
            () => new Queue({ Push() {}, Pop() { return undefined; } }),
            /Queue requires complete Push\/Pop\/Len or push\/pop\/len hooks/
        );
    });
    ...
    test('should work with profiler', () => {
        const profiler = new hsm.Profiler();
        const queue = new Queue(profiler);
        ...
        assert(results.push);
        assert(results.pop);
        assert.strictEqual(results.push.count, 1);
        assert.strictEqual(results.pop.count, 1);
    });
